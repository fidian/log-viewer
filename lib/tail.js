...
class Tail {
    ...
    change(stats) {
        ...
        // It's possible that we don't get a second change with a second
        // immediate write, or that the data isn't ready to be read by the
        // filesystem. Check again after a small delay.
        this.recheckTimeout = setTimeout(() => {
            this.recheckTimeout = null;
            fs.promises.stat(this.path).then((newStats) => this.performChange(newStats));
        }, 1000);
    }

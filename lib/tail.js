...
class Tail {
    ...
    change() {
        ...
        this.taskQueue.push(() => this.performChange());
        this.runTaskQueue();
    }

    initialize() {
        this.newEvent(SYSTEM, "Started tracking file");
        this.taskQueue.push(() => this.performInitialize());
        this.runTaskQueue();
    }
        ...
        return fs.promises
            ...
            .then(
                (stats) => {
                    ...
                    return fs.promises.open(this.path, "r").then(
                        (handle) => {
                ...
                (err) => {
                    console.error(err);
                    this.errored = true;
                    this.newEvent(SYSTEM, "Error reading stats");
    ...
    performChange() {
        ...
        return fs.promises.stat(this.path).then(
            ...
            (err) => {
                console.error(err);
                this.errored = true;
                this.newEvent(SYSTEM, "Error reading stats for a change");
            }
        );
    }
    ...
    readEndOfFile(endSize) {
        return new Promise((resolve) => {
            ...
            stream.on("error", (err) => {
                console.error(err);
                this.errored = true;
                this.newEvent(SYSTEM, "Error reading new content at the end of the file");

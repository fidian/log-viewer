...
class Tail {
    constructor(path, bufferLines, periodicRecheckMin) {
        ...
        this.taskQueue = [];
        this.partialLine = "";
        this.periodicRecheckTimeout = null;
    ...
    performChange(stats) {
        ...
        return this.readEndOfFile();
    }

    periodicRecheck() {
        if (this.periodicRecheckMin === 0) {
            return;
        }

        this.periodicRecheckTimeout = setTimeout(() => {
            this.periodicRecheckTimeout = null;
            fs.promises
                .stat(this.path)
                .then((stats) => this.change(stats))
                .then(() => this.periodicRecheck());
        }, this.periodicRecheckMin * 60000);
    }
    ...
    unlink() {
        if (this.periodicRecheckTimeout) {
            clearTimeout(this.periodicRecheckTimeout);
        }

        this.periodicRecheckMin = 0;
